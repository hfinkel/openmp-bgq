...
// For big-endian systems, the bytes are swapped.
        ...
        void set(int i) override { mask[__kmp_affin_mask_size - i/BITS_PER_MASK_T - 1]
                                     |= ((mask_t)1 << (i % BITS_PER_MASK_T)); }
        bool is_set(int i) const override { return (mask[__kmp_affin_mask_size - i/BITS_PER_MASK_T - 1] &
                                              ((mask_t)1 << (i % BITS_PER_MASK_T))); }
        void clear(int i) override { mask[__kmp_affin_mask_size - i/BITS_PER_MASK_T - 1] &=
                                       ~((mask_t)1 << (i % BITS_PER_MASK_T)); }
#else
